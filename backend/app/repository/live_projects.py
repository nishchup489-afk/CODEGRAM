...
from app.repository.user import get_user_by_clerk_id as _find_user_by_clerk_id
import httpx
...
async def _require_user_by_clerk_id(
    db: AsyncSession,
    clerk_user_id: str,
) -> User:

    user = await _find_user_by_clerk_id(db=db, clerk_user_id=clerk_user_id)
...
# Compatibility name used by existing service tests and callers. All live
# project mutations require a persisted user, while the shared lookup remains
# optional for account synchronization.
get_user_by_clerk_id = _require_user_by_clerk_id
