 ...
 *
 * The hook: a CAFFEINE gauge sits at a critical level. Hovering the support
 * button refuels it to 100% — the hamster perks up, the tear stops falling,
 * steam rises faster. The supporter *feels* their impact before clicking.
 */
...
export default function BuyMeCoffee({
    ...
    const finalCoffeeUrl = coffeeUrl || `https://www.buymeacoffee.com/${username}`;
...
function LegendaryCard({
    ...
    useGSAP(
        () => {
            ...
            /* ── entrance: one orchestrated moment, scroll-triggered ── */
            ...
            entrance
                .from(".bmc-frame", { opacity: 0, y: 36, duration: 0.7 })
                .from(
                    ".bmc-hamster-wrap",
                    { opacity: 0, scale: 0.86, rotate: -6, duration: 0.8, ease: "back.out(1.6)" },
                    "-=0.35",
                )
                .from(
                    ".bmc-reveal",
                    { opacity: 0, y: 18, stagger: 0.09, duration: 0.55 },
                    "-=0.45",
                )
                .fromTo(
                    gaugeFillRef.current,
                    { width: "0%" },
                    { width: `${LOW_CAFFEINE}%`, duration: 1.1, ease: "power2.inOut" },
                    "-=0.2",
                );

            if (reduced.current) return;

            /* ── ambient: hamster breathing ── */
            ...
            /* ── ambient: steam rising from the cup ── */
            gsap.utils.toArray<HTMLElement>(".bmc-steam").forEach((el, i) => {
                gsap.fromTo(
                    el,
                    { y: 0, opacity: 0, scaleX: 0.7 },
                    {
                        y: -44,
                        opacity: 0.55,
                        scaleX: 1.25,
            ...
            /* ── ambient: the tear. drops every few seconds. ── */
            tearTl.current = gsap
                .timeline({ repeat: -1, repeatDelay: 3.2 })
                .fromTo(
                    ".bmc-tear",
                    { y: 0, opacity: 0, scaleY: 0.6 },
                    { opacity: 0.9, scaleY: 1, duration: 0.35, ease: "power1.in" },
                )
                .to(".bmc-tear", {
                    y: 56,
                    opacity: 0,
    ...
    /* ── THE MOMENT: hover the CTA → caffeine refuels, hamster perks up ── */
    ...
    const refuel = contextSafe(() => {
        ...
        if (!reduced.current) {
            tearTl.current?.pause(0); // the crying stops
            ...
            const thanks = scope.current?.querySelector<HTMLElement>(".bmc-thanks");
    ...
    const drain = contextSafe(() => {
        ...
        if (!reduced.current) {
            tearTl.current?.play();
            gsap.to(hamsterRef.current, { rotate: 0, scale: 1, duration: 0.6 });
            gsap.to(".bmc-thanks", { opacity: 0, duration: 0.3 });
        }
    });

    /* magnetic CTA — subtle pull toward the cursor */
    ...
    return (
        ...
        >
            <div className="bmc-frame relative overflow-hidden rounded-3xl border border-[#E8560A]/20 bg-[#0A0908] p-6 sm:p-8">
                {/* ambient glow */}
                ...
                <div className="relative z-10 grid gap-8 sm:grid-cols-[200px_1fr] sm:items-center">
                    ...
                    <div ref={hamsterRef} className="bmc-hamster-wrap relative mx-auto w-44 sm:w-full">
                        <div className="relative overflow-hidden rounded-2xl border border-white/10">
                            <Image
                                ...
                                width={400}
                                height={400}
                                className="bmc-hamster-img h-auto w-full"
                            />
                    ...
                    {/* ── the pitch ── */}
                    <div>
                        <div className="bmc-reveal mb-3 inline-flex items-center gap-2 rounded-full border border-[#E8560A]/25 bg-[#E8560A]/10 px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-orange-200">
                            <Coffee className="h-3 w-3" />
                            Fuel the build
                        </div>

                        <h3
                            className="bmc-reveal text-3xl leading-tight text-white sm:text-4xl"
                            style={{ fontFamily: "'Instrument Serif', 'DM Serif Display', serif" }}
                        >
                            Every line of DevManiac
                            <br />
                            runs on <em className="not-italic text-[#E8560A]">caffeine.</em>
                        </h3>

                        <p className="bmc-reveal mt-3 max-w-md text-sm leading-6 text-zinc-400">
                            Built solo — 4am café shifts, late-night commits, one very
                            tired developer. A coffee keeps the servers (and me) running.
                        </p>

                        {/* ── the caffeine gauge: the thing people remember ── */}
                        <div className="bmc-reveal mt-5 max-w-md">
                            <div className="mb-1.5 flex items-baseline justify-between font-mono text-[10px] uppercase tracking-[0.18em]">
                                <span className="text-zinc-500">caffeine_level</span>
                                <span>
                                    <span ref={gaugePctRef} className="font-bold text-white">
                                        {LOW_CAFFEINE}
                                    </span>
                        ...
                        {/* ── CTAs ── */}
                        <div className="bmc-reveal mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
                            <a
                                ref={ctaRef}
                                href={coffeeUrl}
                                target="_blank"
                                ...
                                onFocus={refuel}
                                onBlur={drain}
                                className="inline-flex w-fit items-center justify-center gap-2 rounded-full bg-[#E8560A] px-6 py-3.5 text-sm font-extrabold text-black transition-shadow duration-300 hover:shadow-[0_0_40px_rgba(232,86,10,0.45)] focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#E8560A]"
                            >
                                <Coffee className="h-4 w-4" />
                                Buy me a coffee — $5
                                <ExternalLink className="h-4 w-4" />
                        ...
                        <p className="bmc-reveal mt-4 font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-600">
                            100% goes to hosting bills + espresso · no subscription
                        </p>
                    </div>
                </div>
            </div>
            ...
            >
                ...
                <div className="relative z-10">
                    ...
                    <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
                        <p className="truncate font-mono text-xs text-zinc-400">{coffeeUrl}</p>
                    </div>
